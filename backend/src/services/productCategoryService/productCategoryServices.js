...
const insertIntoProductCategory = async (name, brandId) => {
  try {
    const insertProductCategory = Productcategory({ name, brandId });
...
const updateProductCategory = async (id, name, brandId) => {
  try {
    const result = await ProductCategorySchema.findByIdAndUpdate(
      id,
      { name, brandId },
